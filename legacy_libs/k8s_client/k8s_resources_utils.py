...
def increase_container_resources_if_necessary(
    ...
    for quantity, requested_quantity in list(requested_quantities.items()):
        ...
        if not requested_quantity or not used_quantity:
            log.warning(f"Not enough information to calculate usage for {quantity=} {container.container_name=}")
            continue
        usage = used_quantity / requested_quantity
        if usage > usage_threshold:
            increased_quantities[quantity] = max(used_quantity, requested_quantity) * increase_factor
            log.info(
...
def calculate_container_average_resources(
    ...
    for pod_in_cluster in cluster_resources.pods_map.values():
        if container.name not in pod_in_cluster.containers:
            continue
        log.info(f"considering existing pod {pod_in_cluster.pod_name} resources on average calculation")
        requested_quantities = pod_in_cluster.containers[container.name].requested_resources.to_quantity_dict()
        for quantity, requested_quantity in list(requested_quantities.items()):
            if not requested_quantity:
                continue
            sum_quantities[quantity] = sum_quantities.get(quantity, 0) + requested_quantity
            count_quantities[quantity] = count_quantities.get(quantity, 0) + 1
    return {quantity: total / count_quantities[quantity] for quantity, total in sum_quantities.items()}
...
def increase_pod_resources_if_necessary(
    ...
    for container in pod.containers:
        quantities_to_apply = {}
        if container_in_cluster := get_container_in_cluster(pod, container.name, cluster_resources):
            log.info(f"Existing Pod {pod.name} checking if resources need to be increased")
            # update pod resources with current requested resources
            pod.container_map[container.name].resources = container_in_cluster.requested_resources
            ...
            if not increased_quantities:
                log.info(f"Pod {pod.name} container {container_in_cluster.container_name} resources are within limits")
                continue
            requested_quantities = container_in_cluster.requested_resources.to_quantity_dict()
            requested_quantities.update(increased_quantities)
            quantities_to_apply = requested_quantities
        elif cluster_resources.pods_map:
            log.info(f"New {pod.name=} {container.name} will be initialized with the average of the other data nodes")
            if quantities_to_apply := calculate_container_average_resources(container, cluster_resources):
                log.info(f"Initializing pod {pod.name} with resource request {quantities_to_apply=}")
...
def apply_all_filters(
    ...
    request_resource = apply_cpu_mem_ratios(request_resource, compute_class=compute_class)
    request_resource = apply_cluster_max_allocatable_limit(
        request_resource, cluster_resources, max_allocatable_threshold=max_cluster_allocatable_threshold
    )
...
def apply_cpu_mem_ratios(
    ...
    if required_resources == k8s_resources.PodResources():
        log.debug(f"requesting minimum resources, no necessary to apply {compute_class=} cpu/mem ratios")
        return required_resources
    # apply compute class minimuls to null values
    if not required_resources.cpu or not required_resources.memory:
        log.warning(f"cpu or mem missing on  {required_resources=} applying {compute_class=} min values")
        required_resources = k8s_resources.PodResources(
    ...
    if required_ratio > compute_class.max_cpu_memory_ratio:
        ...
        log.info(f"{required_resources=} cpu/mem ratio {required_ratio} within {compute_class=}.")
        return required_resources
...
def apply_cluster_max_allocatable_limit(
    ...
    for resource, quantity in required_dict.items():
        if quantity and quantity > (max_allocatable_dict.get(resource) or quantity):
            msg.append(f"{resource=} {quantity=} exceed {max_allocatable_dict[resource]=}")
...
def apply_pod_limits(
    ...
    max_pod_resources = {
        ...
        "ephemeral-storage": float(k8s_resources.parse_quantity(const.MAX_EPHEMERAL_STORAGE)),
    }
    trimmed_quantities = {}
    ...
    if trimmed_quantities:
        ...
        log.warning(f"{required_resources=} exceed {max_pod_resources=} trimmed to {requested_quantities=}")
