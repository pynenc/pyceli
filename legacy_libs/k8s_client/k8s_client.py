...
class Labels(UserDict):
    ...
    def validate_label(key: str, value: str) -> None:
        ...
        if value:
            if len(value) > 63:
                raise ValueError(
                    f"value {value} for label {key} cannot exceed 63 chars"
                )
            if not bool(values_re.fullmatch(value)):
                raise ValueError(
...
class Kubernetes:  # pylint: disable=too-many-public-methods
    ...
    def __init__(self, kubeconfig: Optional[dict] = None) -> None:
        ...
        if kubeconfig:
            log.debug(
                "connection to k8s cluster using kubeconfig and service account credentials"
            )
    ...
    def check_authorization(
        self, group: str, resource: str, verb: str, namespace: str
    ) -> bool:
    ...
    def create_namespace_if_not_exists(self, namespace: str, dry_run: DryRun) -> None:
        ...
        if self.exists_namespace(namespace):
            ...
            self.core_api.create_namespace(
                client.V1Namespace(metadata=namespace_metadata), dry_run=dry_run.value
            )

    def get_all_service_accounts(self, namespace: str) -> Iterator[str]:
        """gets all the service accounts from the current k8s context"""
        for service_account in self.core_api.list_namespaced_service_account(
            namespace
        ).items:
    ...
    def get_service_account(
        name: str, annotations: Optional[dict[str, str]]
        ...
        return client.V1ServiceAccount(
            ...
            metadata=client.V1ObjectMeta(
                name=name,
                labels=Labels({"sa_name": name}).data,
    ...
    def _get_role(
        ...
        return role_cls(
            ...
            kind=kind,
            metadata=client.V1ObjectMeta(
                name=name, labels=Labels({"role_name": name}).data
            ),
            rules=[
                client.V1PolicyRule(
    ...
    def get_role(
        ...
        return Kubernetes._get_role(
            client.V1Role, name, api_group, resource, resource_names, verbs
        )
    ...
    def get_cluster_role(
        ...
        """Get the a Role"""
        return Kubernetes._get_role(
            client.V1ClusterRole, name, api_group, resource, resource_names, verbs
        )
    ...
    @staticmethod
    def get_container_volume_mount(
        name: str, mount_path: str, sub_path: Optional[str]
    ...
    def get_container_spec(
        ...
        if readiness_command:
            readiness_probe = client.V1Probe(
                _exec=client.V1ExecAction(readiness_command),
        ...
        if liveness_pre_stop_command or liveness_post_start_command:
            ...
            if liveness_pre_stop_command:
                pre_stop = client.V1LifecycleHandler(
                    _exec=client.V1ExecAction(command=liveness_pre_stop_command)
                )
            if liveness_post_start_command:
                post_start = client.V1LifecycleHandler(
                    _exec=client.V1ExecAction(command=liveness_post_start_command)
                )
    ...
    def get_pod_template_spec(
        ...
        pod_template = client.V1PodTemplateSpec(
            ...
            metadata=client.V1ObjectMeta(
                name=pod_name, labels=Labels({"pod_name": pod_name, **labels}).data
            ),
        )
        return pod_template
    ...
    def get_job(
        ...
        return client.V1Job(
            ...
            kind="Job",
            metadata=client.V1ObjectMeta(
                name=name, labels=Labels({"job_name": name, **labels}).data
            ),
    ...
    @staticmethod
    def get_cronjob(
        name: str, pod_template: client.V1PodTemplateSpec, schedule: CronTab
    ) -> client.V1CronJob:
        """get the specification of a new job"""
        job = client.V1CronJob(
            ...
            kind="CronJob",
            metadata=client.V1ObjectMeta(
                name=name, labels=Labels({"cronjob_name": name, "component": name}).data
            ),
            spec=client.V1CronJobSpec(
                schedule=schedule,
                job_template=client.V1JobTemplateSpec(
                    spec=client.V1JobSpec(
                        backoff_limit=const.BACKOFF_LIMIT_CRONJOB, template=pod_template
                    )
                ),
    ...
    def get_statefulset(
        ...
        steateful_set = client.V1StatefulSet(
            metadata=client.V1ObjectMeta(
                name=name,
                labels=Labels({"deployment_name": name, "component": name}).data,
            ),
    ...
    @staticmethod
    def get_deployment(
        name: str, pod_template: client.V1PodTemplateSpec, replicas: int
    ) -> client.V1Deployment:
        """get the specification of a new deployment"""
        deployment = client.V1Deployment(
            ...
            spec=client.V1DeploymentSpec(
                ...
                strategy=client.V1DeploymentStrategy(
                    type=DeploymentStrategyType.RECREATE.value
                ),
            ),
        )
        return deployment
    ...
    @staticmethod
    def get_service(
        name: str, ports: list[client.V1ServicePort], selector: dict
    ) -> client.V1Service:
        """get a service"""
        return client.V1Service(
            ...
            kind="Service",
            metadata=client.V1ObjectMeta(
                name=name, labels=Labels({"service": name, "component": name}).data
            ),
    ...
    def get_hpa(
        ...
        return client.V2HorizontalPodAutoscaler(
            ...
            metadata=client.V1ObjectMeta(
                name=name, labels=Labels({"hpa_name": name, "component": name}).data
            ),
                    ...
                    client.V2MetricSpec(
                        ...
                        resource=client.V2ResourceMetricSource(
                            ...
                            target=client.V2MetricTarget(
                                type="Utilization",
                                average_utilization=target_cpu_utilization_percentage,
                            ),
                        ),
                    )
                ],
    ...
    def get_vpa(
        ...
        if not control_cpu or not control_memory:
            update_policy["controlledResources"] = (
                ["cpu"] if control_cpu else ["memory"]
            )
        if min_allowed or max_allowed:
            container_policy: dict = (
                {"containerName": container_name} if container_name else {}
            )
            if min_allowed:
                container_policy["minAllowed"] = {
                    k: v
                    for k, v in min_allowed.to_dict().items()
                    if k != "ephemeral-storage"
                }
            if max_allowed:
                container_policy["maxAllowed"] = {
                    k: v
                    for k, v in max_allowed.to_dict().items()
                    if k != "ephemeral-storage"
                }
    ...
    def get_persistent_volume(
        name: str, storage: str, disk_name: str
        ...
        return client.V1PersistentVolume(
            ...
            metadata=client.V1ObjectMeta(
                name=name,
                labels=Labels(
                    {"volume": name, "component": name.replace("-pv", "")}
                ).data,
            ),
            spec=client.V1PersistentVolumeSpec(
                ...
                gce_persistent_disk=client.V1GCEPersistentDiskVolumeSource(
                    pd_name=disk_name, fs_type="ext4"
                ),
            ),
        )
    ...
    def get_persistent_volume_claim(
        ...
        return client.V1PersistentVolumeClaim(
            ...
            metadata=client.V1ObjectMeta(
                name=name, labels=Labels({"component": name.replace("-pvc", "")}).data
            ),
    ...
    @staticmethod
    def get_persistent_volume_claim_template(
        name: str, storage: str
    ...
    def upsert_envvars(
        self, base_env: list[client.V1EnvVar], new_env: list[client.V1EnvVar]
        ...
        for new_item in new_env:
            if new_item.name in env_map:
                ...
                log.debug(
                    f"Adding new {self.describe_envvar(new_item)} to environment variables"
                )
    ...
    def get_env_from_source(self, item_names: List[str]) -> List[client.V1EnvVar]:
        ...
        def add_to_list(item: client.V1Secret | client.V1ConfigMap) -> None:
            if (name := item.metadata.name) in item_names:
                log.debug(
                    "adding %s %s to the environment variables",
                    type(item).__name__,
                    name,
                )
    ...
    @staticmethod
    def get_suspend_cronjob_body(
        cronjob: client.V1CronJob, suspend: bool
    ) -> client.V1CronJob:
    ...
    def _change_all_cronjobs_suspend(
        self, namespace: str, suspend: bool, async_req: bool, dry_run: DryRun
    ) -> None:
        for cronjob in self.batch_api.list_namespaced_cron_job(namespace).items:
            if bool(cronjob.spec.suspend) != suspend:
                cronjob.spec.suspend = suspend
                log.info(
                    "changing CronJob %s suspent to %s", cronjob.metadata.name, suspend
                )
    ...
    def enable_all_cronjobs(
        self, namespace: str, async_req: bool = False, dry_run: DryRun = DryRun.OFF
    ) -> None:
    ...
    def get_cluster_pods(
        self, namescape: str, label_selector: Optional[dict[str, str]] = None
    ) -> list[client.V1Pod]:
        """get the cluster resources"""
        ...
        return self.core_api.list_namespaced_pod(
            namespace=namescape, label_selector=_label_selector
        ).items
