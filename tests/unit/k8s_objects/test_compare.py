...
def test_find_differences_no_changes() -> None:
    ...
    assert compare.find_differences(desired_spec, existing_spec).considered == []
...
def test_find_differences_with_changes() -> None:
    ...
    existing_spec = {"key1": "value1", "key2": "value2"}
    assert compare.find_differences(desired_spec, existing_spec).considered == [
        compare.PathComparison(("key1",), "value1", "new_value")
    ]
...
def test_find_differences_with_nested_changes() -> None:
    ...
    assert compare.find_differences(desired_spec, existing_spec).considered == [
        compare.PathComparison(("key1", "nested_key"), "old_value", "new_value")
    ]
...
def test_determine_update_action_equals() -> None:
    ...
    existing = {"key": "value"}
    assert (
        compare._determine_update_action("anyKind", desired, existing).update_action
        == compare.UpdateAction.EQUALS
    )
...
def test_determine_update_action_needs_patch() -> None:
    ...
    assert (
        compare._determine_update_action("anyKind", desired, existing).update_action
        == compare.UpdateAction.NEEDS_PATCH
    )
...
def test_determine_update_action_needs_replacement() -> None:
    ...
    assert (
        compare._determine_update_action("anyKind", desired, existing).update_action
        == compare.UpdateAction.NEEDS_REPLACEMENT
    )
...
def test_requires_replacement_for_subpath() -> None:
    ...
    path_immutable = ("spec", "template", "metadata", "labels")
    assert compare.requires_replacement("AnyKind", path_immutable) is True

    # A mutable path
    path_mutable = ("spec", "containers", "image")
    assert compare.requires_replacement("AnyKind", path_mutable) is False

    # Subpath of an immutable field but not starting with it should not require replacement
    path_not_starting_with_immutable = ("metadata", "spec", "selector")
    assert (
        compare.requires_replacement("AnyKind", path_not_starting_with_immutable)
        is False
    )

    # Check an exact immutable field
    path_exact_immutable = ("spec", "selector")
    assert compare.requires_replacement("AnyKind", path_exact_immutable) is True


def test_required_replacement_for_pvc_resource() -> None:
    """PVC spec is immutable except for storage requests"""
    path = ("spec", "resources", "requests", "storage")
    assert compare.requires_replacement("PersistentVolumeClaim", path) is False

    path = ("spec", "resources", "requests", "whatever")
    assert compare.requires_replacement("PersistentVolumeClaim", path) is False

    path = ("spec", "other")
    assert compare.requires_replacement("PersistentVolumeClaim", path) is True
...
def test_find_differences_with_ignored_paths() -> None:
    ...
    assert set(differences.ignored) == {
        ...
        compare.PathComparison(("spec", "finalizers"), ["kubernetes"], None),
...
def test_find_differences_with_none_and_string() -> None:
    ...
    assert differences.considered == [
        compare.PathComparison(("key",), "existing_value", None)
